...
subtitle: "Standard Length, Normalised Adiposity, Morphology Effect, Droplet Count, Droplet Area, and Depot vs Morphology Scatter"
author: "Minchin Lab"
date: "`r Sys.Date()`"
output:
  html_document:
...
6. **Depot Size vs Morphology Scatter** - Relationship between depot lipid area and cell morphology
...
p_morph <- create_forest_plot(
  ...
  coord_cartesian(xlim = c(-100, 100))  # Restrict x-axis to ±100%
...
# Analysis 6: Depot Size vs Morphology Scatter Plot

This scatter plot combines the morphology LMM effect (Analysis 3) with depot lipid area to visualise the relationship between **how much fat** (depot size) and **what kind of fat** (cell morphology) each gene produces.

- **X-axis:** Morphology value from the LMM intercept (identical to Analysis 3 forest plot)
- **Y-axis:** Total depot lipid area (% change from controls, simple mean)
- **Point size:** Number of mutant fish screened

The four quadrants distinguish distinct biological strategies:

- **Top-right:** Larger depot with hypertrophic cells (more fat in bigger droplets)
- **Top-left:** Larger depot with hyperplastic cells (more fat in more/smaller droplets)
- **Bottom-right:** Smaller depot with hypertrophic cells
- **Bottom-left:** Smaller depot with hyperplastic cells

```{r scatter-data}
# =============================================================================
# DEPOT SIZE vs MORPHOLOGY SCATTER PLOT
# =============================================================================
#
# X-axis: Morphology LMM intercept from Analysis 3 (already computed as morph_stats)
# Y-axis: Total depot lipid area (% change from controls)
#
# This reuses morph_stats from the morphology analysis above (morph_lmm_results),
# ensuring the x-axis values are identical to the forest plot diamonds.
# =============================================================================

# Depot area: simple % change from controls
ctrl_depot <- morph_data %>%
  filter(genotype == "control", !is.na(total_adipose_area), total_adipose_area > 0) %>%
  group_by(gene) %>%
  summarise(ctrl_depot_area = mean(total_adipose_area, na.rm = TRUE), .groups = "drop")

mut_depot <- morph_data %>%
  filter(genotype == "mutant", !is.na(total_adipose_area), total_adipose_area > 0) %>%
  group_by(gene) %>%
  summarise(mut_depot_area = mean(total_adipose_area, na.rm = TRUE),
            n_mut = n(), .groups = "drop")

depot_df <- mut_depot %>%
  left_join(ctrl_depot, by = "gene") %>%
  mutate(depot_area_effect = (mut_depot_area - ctrl_depot_area) / ctrl_depot_area * 100)

# Merge with morphology LMM results (from Analysis 3)
scatter_df <- morph_lmm_results %>%
  select(gene, morph_lmm = intercept_pct, n = n_mut) %>%
  left_join(depot_df %>% select(gene, depot_area_effect), by = "gene")

cat("Scatter plot data:\n")
print(scatter_df %>% arrange(desc(morph_lmm)))
```

```{r scatter-plot, fig.width=8, fig.height=6.5}
library(ggrepel)

p_scatter <- ggplot(scatter_df, aes(x = morph_lmm, y = depot_area_effect)) +
  geom_hline(yintercept = 0, colour = "grey70", linewidth = 0.5) +
  geom_vline(xintercept = 0, colour = "grey70", linewidth = 0.5) +
  geom_point(aes(size = n), colour = "#2171B5", alpha = 0.7) +
  geom_text_repel(aes(label = gene), size = 3, fontface = "italic",
                  colour = "#333333", segment.colour = "#AAAAAA",
                  segment.size = 0.3, max.overlaps = Inf) +
  scale_size_continuous(range = c(2, 8), name = "Sample size",
                        breaks = c(15, 30, 60)) +
  # Quadrant annotations
  annotate("label", x = min(scatter_df$morph_lmm, na.rm = TRUE) * 0.85,
           y = max(scatter_df$depot_area_effect, na.rm = TRUE) * 0.95,
           label = "Hyperplastic cells\nLarger depot",
           size = 2.8, colour = "#666666", fill = "#F7F7F7",
           label.size = 0.3, label.r = unit(0.2, "lines")) +
  annotate("label", x = max(scatter_df$morph_lmm, na.rm = TRUE) * 0.85,
           y = max(scatter_df$depot_area_effect, na.rm = TRUE) * 0.95,
           label = "Hypertrophic cells\nLarger depot",
           size = 2.8, colour = "#666666", fill = "#F7F7F7",
           label.size = 0.3, label.r = unit(0.2, "lines")) +
  annotate("label", x = min(scatter_df$morph_lmm, na.rm = TRUE) * 0.85,
           y = min(scatter_df$depot_area_effect, na.rm = TRUE) * 0.95,
           label = "Hyperplastic cells\nSmaller depot",
           size = 2.8, colour = "#666666", fill = "#F7F7F7",
           label.size = 0.3, label.r = unit(0.2, "lines")) +
  annotate("label", x = max(scatter_df$morph_lmm, na.rm = TRUE) * 0.85,
           ...
           size = 2.8, colour = "#666666", fill = "#F7F7F7",
           label.size = 0.3, label.r = unit(0.2, "lines")) +
  labs(x = "Morphology value (% change from controls)",
       y = "Depot lipid area (% change from controls)") +
...
# Save
ggsave("depot_vs_morphology_scatter.svg", p_scatter, width = 8, height = 6.5)
ggsave("depot_vs_morphology_scatter.png", p_scatter, width = 8, height = 6.5, dpi = 300, bg = "white")
```
...
cat("\nScatter plot saved as SVG and PNG (8\" x 6.5\"):\n")
cat("- depot_vs_morphology_scatter.svg/png\n")
```
