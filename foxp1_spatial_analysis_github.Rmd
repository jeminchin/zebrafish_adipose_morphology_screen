...
| **HFD imaging** | 49 dpf |
| **Spatial bins** | 200 µm strata from oldest (anterior) lipid droplets |
...
ggplot(foxp1b_avg, aes(x = as.numeric(Stratum), y = Group_Mean_Feret, color = Simplified_Group)) +
  ...
  labs(title = "FOXP1B: Mean Feret Diameter by Stratum",
       ...
       x = "Stratum (from oldest/leftmost)", y = "Mean Feret Diameter (µm)", color = "Group")
```
...
ggplot() +
  ...
  labs(title = "FOXP1B: Individual Fish with Group Means",
       ...
       x = "Stratum", y = "Mean Feret Diameter (µm)", color = "Group")
```
...
**Model formula:** `Mean_Feret ~ Diet × Genotype × Stratum_numeric + (1|Fish_Pair)`
...
- **Diet × Genotype interaction**: F = `r round(anova_foxp1b$"F value"[4], 2)`, p = `r format.pval(anova_foxp1b$"Pr(>F)"[4], digits = 3)`
- **Diet × Genotype × Stratum**: F = `r round(anova_foxp1b$"F value"[7], 2)`, p = `r format.pval(anova_foxp1b$"Pr(>F)"[7], digits = 3)`
...
```{r foxp1b-mut-diet}
mut_diet_b %>%
  select(Stratum_numeric, estimate, SE, p.value) %>%
  mutate(Significant = ifelse(p.value < 0.05, "Yes", "No")) %>%
  kable(digits = 3, caption = "Diet effect in mutants across strata")
```

```{r foxp1b-mut-summary, echo=FALSE}
mut_sig_strata_b <- mut_diet_b$Stratum_numeric[mut_diet_b$p.value < 0.05]
```

**Summary**: HFD significantly increases droplet size in mutants at strata `r paste(mut_sig_strata_b, collapse = ", ")` (p < 0.05), but **not in later strata** -  effect is only observed in more mature regions of the adipose tissue.
...
wt_diet_b %>%
  select(Stratum_numeric, estimate, SE, p.value) %>%
  mutate(Significant = ifelse(p.value < 0.05, "Yes", "No")) %>%
  kable(digits = 4, caption = "Diet effect in WT across strata")
```

```{r foxp1b-wt-summary, echo=FALSE}
wt_sig_strata_b <- wt_diet_b$Stratum_numeric[wt_diet_b$p.value < 0.05]
```

**Summary**: HFD significantly increases droplet size in WT fish across **`r ifelse(length(wt_sig_strata_b) == 15, "ALL", length(wt_sig_strata_b))` strata** (`r paste(wt_sig_strata_b, collapse = ", ")`) - the effect is sustained throughout all regions of the adipose tissue.
...
geno_con_b <- geno_contrasts_b[geno_contrasts_b$Diet == "Control", ] %>%
  mutate(Genotype_Effect = estimate, Significant = ifelse(p.value < 0.05, "Yes", "No"))
...
```{r foxp1b-geno-con}
geno_con_b %>%
  select(Stratum_numeric, Genotype_Effect, SE, p.value, Significant) %>%
  kable(digits = 3, caption = "Genotype effect (Mutant - WT) on control diet")
```

```{r foxp1b-geno-con-summary, echo=FALSE}
geno_con_sig_b <- geno_con_b$Stratum_numeric[geno_con_b$p.value < 0.05]
geno_con_early_b <- mean(geno_con_b$Genotype_Effect[geno_con_b$Stratum_numeric <= 5])
```

**Summary**: Mutants have significantly larger droplets than WT on control diet across **`r length(geno_con_sig_b)` strata** (`r paste(geno_con_sig_b, collapse = ", ")`). The effect is ~`r round(geno_con_early_b, 0)` µm in early strata.
...
geno_hfd_b %>%
  select(Stratum_numeric, Genotype_Effect, SE, p.value, Significant) %>%
  kable(digits = 3, caption = "Genotype effect (Mutant - WT) on HFD")
```

```{r foxp1b-geno-hfd-summary, echo=FALSE}
geno_hfd_sig_b <- geno_hfd_b$Stratum_numeric[geno_hfd_b$p.value < 0.05]
```

**Summary**: On HFD, the genotype effect **reverses** - mutants start slightly larger in early strata but WT becomes larger in late strata (`r if(length(geno_hfd_sig_b) > 0) paste("significant at strata", paste(geno_hfd_sig_b, collapse = ", ")) else "no significant differences"`). This is because WT fish mount a sustained HFD response while mutants do not.
...
) %>% mutate(Diet_Effect = -estimate, Significant = ifelse(p.value < 0.05, "Yes", "No"))
...
ggplot(diet_effects_b, aes(x = Stratum_numeric, y = Diet_Effect, color = Genotype)) +
  ...
  labs(title = "FOXP1B: HFD Effect on Diameter",
       subtitle = "Positive = HFD increases size; filled = p < 0.05",
       x = "Stratum", y = "HFD Effect (µm)", color = "Genotype", shape = "Significant",
...
**Interpretation**: WT fish show a strong, persistent HFD response (~`r round(wt_early_effect_b, 0)` µm in early strata, ~`r round(wt_late_effect_b, 0)` µm in late strata). Mutants show a **weaker, transient response** (~`r round(mut_early_effect_b, 0)` µm in early strata, declining to ~`r round(mut_late_effect_b, 0)` µm by late strata). This represents an **~`r attenuation_b`% attenuation** of the HFD response in mutants.
...
) %>% mutate(Effect_Size = -estimate, Significant = ifelse(p.value < 0.05, "Yes", "No"))

geno_comb_b <- rbind(
  geno_con_b %>% mutate(Effect_Type = "Genotype (Control)"),
  geno_hfd_b %>% mutate(Effect_Type = "Genotype (HFD)")
) %>% select(Stratum_numeric, Effect_Type, Effect_Size = Genotype_Effect, SE, p.value, Significant)

all_effects_b <- rbind(
  diet_comb_b %>% select(Stratum_numeric, Effect_Type, Effect_Size, SE, p.value, Significant),
  geno_comb_b
)
...
ggplot(all_effects_b, aes(x = Stratum_numeric, y = Effect_Size, color = Effect_Type)) +
  ...
  labs(title = "FOXP1B: All Diet and Genotype Effects",
       subtitle = "Filled = p < 0.05",
       x = "Stratum", y = "Effect (µm)", color = "Effect Type", shape = "Significant",
       caption = "Filled points indicate p < 0.05")
```
...
1. **Full Linear**: `Diet × Genotype × Stratum` (three-way interaction)
2. **Simple Linear**: `Diet × Genotype + Diet × Stratum + Genotype × Stratum` (no three-way)
3. **Categorical**: `Diet × Genotype × Stratum_factor` (stratum as factor)
...
comparison_anova_b <- data.frame(
  Interaction = c("Diet × Genotype", "Diet × Stratum", "Genotype × Stratum"),
...
1. **Baseline genotype effect**: foxp1b mutants have **constitutively larger lipid droplets** than WT on control diet (~`r baseline_effect_b` µm difference in early strata, significant across `r length(geno_con_sig_b)` strata)

2. **Attenuated HFD response in mutants**: The foxp1b mutation reduces HFD sensitivity by approximately **`r attenuation_b`%** (`r round(mut_early_effect_b, 0)` µm vs `r round(wt_early_effect_b, 0)` µm effect in early strata)
...
- **Diet × Genotype interaction**: F = `r round(anova_foxp1a$"F value"[4], 2)`, p = `r format.pval(anova_foxp1a$"Pr(>F)"[4], digits = 3)`
- **Diet × Genotype × Stratum**: F = `r round(anova_foxp1a$"F value"[7], 2)`, p = `r format.pval(anova_foxp1a$"Pr(>F)"[7], digits = 3)`
...
mut_sig_strata_a <- mut_diet_a$Stratum_numeric[mut_diet_a$p.value < 0.05]
```

**Summary**: `r if(length(mut_sig_strata_a) > 0) paste("HFD significantly affects droplet size in foxp1a mutants at strata", paste(mut_sig_strata_a, collapse = ", "), "(p < 0.05).") else "**No significant diet effect in foxp1a mutants at any stratum** - the mutation abolishes the normal spatial gradient of HFD response."` Unlike WT (and foxp1b mutants), foxp1a mutants show a **flattened, uniform response** across all strata.
...
wt_sig_strata_a <- wt_diet_a$Stratum_numeric[wt_diet_a$p.value < 0.05]
```

**Summary**: `r if(length(wt_sig_strata_a) > 0) paste("HFD significantly affects droplet size in WT fish at strata", paste(wt_sig_strata_a, collapse = ", "), "- showing the expected spatial gradient with stronger effects in mature (early) strata.") else "No significant diet effect in WT at any stratum."` WT fish maintain normal **spatial patterning** of the HFD response.
...
geno_con_sig_a <- geno_con_a$Stratum_numeric[geno_con_a$p.value < 0.05]
...
**Summary**: `r if(length(geno_con_sig_a) > 0) paste("Significant genotype effect at strata", paste(geno_con_sig_a, collapse = ", "), ". foxp1a mutants have larger adipocytes on control diet (~", round(geno_con_early_a, 0), "µm in early strata, ~", round(geno_con_late_a, 0), "µm in late strata).") else paste("No significant genotype effect on control diet, though there is a trend toward larger adipocytes in mutants (~", round(geno_con_early_a, 0), "µm in early strata).")`
...
geno_hfd_sig_a <- geno_hfd_a$Stratum_numeric[geno_hfd_a$p.value < 0.05]
...
**Summary**: `r if(length(geno_hfd_sig_a) > 0) paste("On HFD, significant genotype effect at strata", paste(geno_hfd_sig_a, collapse = ", "), ".") else "On HFD, no significant genotype differences."` Notably, the genotype effect is **relatively uniform across strata** (~`r round(geno_hfd_early_a, 0)` µm early, ~`r round(geno_hfd_late_a, 0)` µm late), reflecting the **loss of spatial patterning** in foxp1a mutants.
...
- **WT**: Shows the expected **spatial gradient** - stronger HFD response in early/mature strata (~`r round(wt_early_effect_a, 0)` µm) declining toward late/young strata (~`r round(wt_late_effect_a, 0)` µm).
- **Mutant**: Shows a **flattened, uniform response** across all strata (~`r round(mut_early_effect_a, 0)` µm early, ~`r round(mut_late_effect_a, 0)` µm late) - the normal spatial gradient is **abolished**.
- **Attenuation**: In early strata, the mutant HFD response is **~`r attenuation_early_a`% reduced** compared to WT (`r round(mut_early_effect_a, 0)` µm vs `r round(wt_early_effect_a, 0)` µm).
...
ggplot(geno_con_a, aes(x = Stratum_numeric, y = Genotype_Effect)) +
  ...
  geom_point(aes(shape = Significant, size = Significant), color = "#99D6D6") +
  ...
  labs(title = "FOXP1A: Genotype Effect on Control Diet",
       subtitle = "Positive = Mutant larger than WT; filled = p < 0.05",
       x = "Stratum", y = "Genotype Effect (µm)\n(Mutant - WT)",
...
all_effects_a <- rbind(
  diet_comb_a %>% select(Stratum_numeric, Effect_Type, Effect_Size, SE, p.value, Significant),
  geno_comb_a
)
...
1. **Loss of spatial patterning**: foxp1a mutants show a **uniform, flattened HFD response** across all strata. WT fish maintain the normal spatial gradient (~`r round(wt_gradient_a, 0)` µm difference between early and late strata), while mutants show only ~`r round(mut_gradient_a, 0)` µm difference - a **~`r gradient_loss_a`% reduction** in spatial information.

2. **Attenuated HFD response**: The foxp1a mutation reduces the HFD response in early strata by **~`r attenuation_a`%** (`r round(mut_early_effect_a, 0)` µm vs `r round(wt_early_effect_a, 0)` µm in WT).

3. **Baseline genotype effect**: foxp1a mutants have constitutively larger lipid droplets on control diet (~`r round(geno_con_early_a, 0)` µm difference), which was not statistically significant. Hypertrophy at baseline is much lower than foxp1b mutants.
...
comparison_table <- data.frame(
  ...
  foxp1a = c(
    paste0("~", round(geno_con_early_a, 0), " µm"),
  ...
  foxp1b = c(
    paste0("~", round(geno_con_early_b, 0), " µm"),
...
ggplot(diet_effects_comparison, aes(x = Stratum_numeric, y = Diet_Effect, 
  ...
  labs(title = "Direct Comparison: HFD Effect in foxp1a vs foxp1b",
       subtitle = "foxp1a shows uniform/flat response; foxp1b shows transient (early-only) response",
       x = "Stratum (from oldest/leftmost)", y = "HFD Effect (µm)", 
       color = "Genotype", shape = "Significant")
```
